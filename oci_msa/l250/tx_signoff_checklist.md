...
| Electrical eye mask (horizontal / vertical opening) | $X_1$, $X_2$, $Y_1$, $Y_2$ | Provisional: 0.14 UI / 0.40 UI / 350 mV / 1500 mV | `TBD_from_link_budget` | Normative definition in `architecture_spec.md` §4-5 (geometry, alignment, pass/fail committed; coordinates provisional; machine-readable `tx_eye_mask.json`). CEI-56G-XSR-NRZ reference: X1=0.14 UI, X2=0.4 UI, Y1=125 mV, Y2=200 mV (§9.2) |
...
- [ ] **Finalize the provisional eye-mask coordinate values.** $Y_1$ (350 mV) must be re-derived from the MRM $P(V)$ transfer so the inner opening guarantees the §8-2 ER/OMA/TDEC targets; $X_2$ (0.40 UI) must be validated against the 3.2–4.5 ps transition-time envelope; $X_1$ (0.14 UI) tracks the closed jitter budget; $Y_2$ (1500 mV = $V_{PP,max}/2$) must be confirmed against MRM reverse-bias reliability limits. Owner: link budget / PIC / SI. Basis: `TBD_from_link_budget`.
- [ ] **Finalize eye-mask pass/fail statistics and BER convention.** Confirm the provisional ≥10⁶ UI per-corner observation length and decide between strict zero-hit and a hit-ratio (BER-equivalent) allowance. Note (spec §4-5 decomposition): the §3-1 allocations close $X_1$ with +0.015 UI margin only at the pre-FEC clause BER 2.4e-4; at 1e-15 the RJ term alone grows to 0.175 UI and violates $X_1$ by 0.083 UI, so the mask BER convention is load-bearing. Owner: link budget / validation. Basis: `TBD_from_link_budget`.
- [ ] **Allocate a vertical eye-closure budget.** The §4-5 decomposition shows 567 mV (57% of the min-swing rail) between $Y_1$ and the rail is unattributed — only SNDR-bounded noise/distortion (83 mV at clause BER) is budgeted. Add allocations for worst-case residual ISI amplitude, overshoot ripple, level mismatch, and swing/PVT derating; this is a prerequisite for deriving a defensible $Y_1$. Owner: link budget / SI. Basis: `TBD_from_link_budget`.
