...
**Allocation set by the dj `JH4u` ceiling, not by the chain build-up.** The spec resolves the §7 `JH4u` tension by the tougher-spec rule (spec §2-3): at BUJ = 0.036 UI, meeting `JH4u` ≤ 0.118 UI pp requires $\sigma_{RJ} \le 0.011$ UI, superseding the earlier 0.015 UI allocation and its 0.018 UI fallback — both fail the dj clock-jitter screen. The first-cut chain build-up above lands at ≈142 fs, 1.37× over the new allocation: the chain must improve by ≈2.7 dB in integrated phase-noise power (e.g. a ≈72/45/50/30 fs re-partition, RSS ≈ 103 fs), and that re-partition is `TBD_analog_design`. The kill-or-confirm flag hardens accordingly — this is now the budget's binding design risk. Cross-check: dj `JHRMS` ≤ 0.023 UI rms is the standards analog (clock-only, slope-extrapolated) — the target is 52% inside it.
...
| *(superseded)* no-FIR at the old $\sigma_{RJ}=0.015$ | 0.073 | 0.015 | 0.284 UI | 2.67 |
...
The `JH4u` tension is resolved by the tougher-spec rule (spec §2-3): the JH family excludes data-correlated jitter, so its internal analog is BUJ plus the RJ tail at 1E-4; at the old $\sigma_{RJ} = 0.015$ UI this was 0.148 UI — 0.03 UI over the dj ceiling. The budget adopts the ceiling as binding, tightening $\sigma_{RJ}$ to 0.011 UI (104 fs) at unchanged BUJ = 0.036 UI and landing exactly on `JH4u` = 0.118 UI. The consequence lives in §2: the first-cut clock-chain build-up (142 fs) no longer fits and must be re-partitioned — the remaining kill-or-confirm (`TBD_from_sim_sweep`).
...
1. **TIE extraction.** Transient simulation with the extracted driver + 60 fF MRM-plus-pad load, PRBS13 and PRBS31, all corners; record time-interval error of each differential mid-level crossing at TP1 against the ideal serializer clock (same folding convention as the §3-4 mask — recovered-clock alignment prohibited).
