...
| **Vertical eye-mask coordinates (provisional)** | $Y_1$, $Y_2$ | $Y_1=350$ mV; $Y_2=1500$ mV | $Y_1$ = inner half-opening required to drive the MRM to the specified extinction ratio (placeholder pending MRM $P(V)$ derivation); $Y_2$ = maximum-amplitude bound = $V_{PP,max}/2$ hard-clip level (§4-1). Full mask definition in §4-5 (`TBD_from_link_budget`). |
...
1. **Inner keep-out hexagon** — no sample of $v(t)$ may fall inside (or on the boundary of) the polygon with vertices, in $(t\ \mathrm{[UI]},\ v\ \mathrm{[mV]})$:
   ...
   With the provisional coordinates this is $(0.14, 0)$, $(0.40, +350)$, $(0.60, +350)$, $(0.86, 0)$, $(0.60, -350)$, $(0.40, -350)$.
...
| $Y_1$ | 350 mV | Placeholder with no standards derivation. Final value must be derived from the MRM $P(V)$ transfer at the committed bias so that an eye open beyond $\pm Y_1$ guarantees the §8-2 ER ≥ 3.5 dB, OMA, and TDEC targets with PVT and noise margin (`TBD_from_link_budget`). |
...
Random jitter dominates the horizontal budget (55% of $X_1$), followed by ISI (16%), BUJ (13%), and DCD (5%). Two consequences: (i) $X_1$ is BER-sensitive — the same allocations evaluated at $10^{-15}$ give $Q=7.94$, an RJ term of 0.175 UI, and a total closure of 0.223 UI per side, **exceeding $X_1$ by 0.083 UI**. The provisional coordinates are therefore only self-consistent at the pre-FEC clause BER, and the pass/fail statistics above must fix the mask's BER convention accordingly. (ii) Improving DCD or ISI buys comparatively little horizontal margin relative to the RJ allocation.

*Vertical (partial — exposes a budget gap).* The only committed vertical-error bound is SNDR ≥ 32.5 dB. Interpreted provisionally as an rms error against the minimum-swing rail $A=V_{PP,min}/2=1000$ mV, it accounts for $\sigma=23.7$ mV, i.e. $Q\sigma=83$ mV of closure at the clause BER. Stacking rail − noise − $Y_1$ leaves **567 mV (57% of the rail) unallocated**: the spec currently has no vertical ISI / ripple / level-mismatch budget row, so the majority of the vertical eye between $Y_1$ and the rail is unattributed. This is the quantitative statement of why $Y_1$ is a placeholder — closing it requires a vertical eye-closure allocation (worst-case residual ISI amplitude, overshoot ripple, and swing/PVT derating) alongside the MRM $P(V)$ derivation (`TBD_from_link_budget`).
