...
| File | What it is |
|---|---|
| `OCI_Link_Budget_Report.md` | The unified engineering report — methodology, derivations, requirements, risks (source of truth; `OCI_Link_Budget_Report.pdf` is a rendered export and may lag it) |
| `OCI_Link_Budget_Summary.md` | Results-only digest: tables and figures, minimal commentary |
| `Methodology_Provenance.md` | Companion to the report: every calculation methodology classified by provenance tier (reference / hybrid / derived-here), plus Appendix C with worked arithmetic behind each executive-summary number |
| `OCI-GEN2_Simplified.png` | GEN2 architecture block diagram (reproduced in report §4.1) |
| `fig_waterfall.png`, `fig_tornado.png` | Summary figures (GEN2 budget waterfall, margin sensitivity) — regenerate with `07_summary_figures.py` |

## Setup

```bash
pip install -r requirements.txt   # numpy, pandas, scipy (+ matplotlib for 07)
python 01_tia_survey.py           # etc. — each script is standalone
```

No arguments; runtime is seconds for 01–03/05/07, ~1 min for 04 and 06 (CTLE/FIR grid
...
| `04_gen2_106g_feasibility.py` | 106.25 GBd rework: 152-setting rescan, noise scaling law, required TIA class, closure | f²-fit R² 0.87 vs 0.59; scaled floors −10.60/−5.63 dBm; target-class floor **−11.41 dBm**; ISI+EQ 0.82/1.15/1.70; stack **4.07 dB** typ; margins **+1.67/+1.34/+0.79**; required iₙ 4.03 µA; CDR-fallback jitter check (169 fs: **+0.19 dB**) (spec canvas §2–6: 3.74/+1.67 etc., pre-convention) |
...
| `07_summary_figures.py` | Figures for the results-only summary | `fig_waterfall.png` (GEN2 budget waterfall), `fig_tornado.png` (margin sensitivity) |
