...
def generate_eye_from_ir(ir_continuous):
    # Scale continuous impulse response to discrete by multiplying by DT
    ir_discrete = ir_continuous * DT
    ...
    # Apply Zero-Order Hold (ZOH) filter
    zoh = np.ones(SPS)
    waveform_zoh = np.convolve(symbols_up, zoh, mode='full')
    
    # Convolve with discrete impulse response
    waveform = np.convolve(waveform_zoh, ir_discrete, mode='full')
...
def process_csv(csv_file, output_dir):
    ...
    def _plot_subset(cols, polarity):
        ...
        for ax, (x_col, y_col) in zip(axes, cols):
            ...
            t_ui, segments = generate_eye_from_ir(dy)
            
            for seg in segments:
                ax.plot(t_ui, seg, color='b', alpha=0.05, linewidth=0.5)
                
            ax.set_title(f"Eye Diagram (from Step IR): {label}")
        ...
        fig.suptitle(f"{base_name} ({polarity}) - PAM4 Eye Diagrams from Step IR")
        fig.tight_layout()
...
def main():
    ...
    for csv_file in csv_files:
        ...
        output_dir = os.path.join("temp/plots_eyes_ir/step_response", dir_name)
